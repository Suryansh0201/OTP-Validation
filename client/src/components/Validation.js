...
import { useValidationUserMutation, useVerifyOTPMutation } from '../Services/OTPApi';
...
const Validation = () => {
  ...
  // RTK Query
  const [ValidationUser] = useValidationUserMutation()
  ...
  const handleSendOTPForm = async (e) => {
    ...
    if (actualData.phonenumber) {
      const res = await ValidationUser(actualData)
  ...
  return (
    <>
      ...
      {authid === null ?
        <>
          <Box component="form" sx={{ p: 3, display: 'flex', justifyContent: 'center' }} noValidate id="Validation-form" onSubmit={handleSendOTPForm}>
